...
#[derive(Deserialize)]
pub struct TryAgentWsQuery {
    pub agent_id: String,
}

/// One-way STT->MT->TTS demo, self-looped back to the same caller — this is
/// what the try-agent screen talks to, not the two-leg (mobile + Twilio)
/// call flow, so there's no `CallRegistry`/orchestration involved here.
pub async fn try_agent_ws(
    Extension(session): Extension<UserSession>,
    Query(query): Query<TryAgentWsQuery>,
    ws: WebSocketUpgrade,
) -> Result<Response, ApiResponse<()>> {
    tracing::info!(user_id = %session.user_id, "try_agent_ws: request from authenticated user");
    ...
    let agent = agents::Entity::find_by_id(agent_id)
        ...
        .await
        .map_err(|e| {
            tracing::error!("try_agent_ws: failed to look up agent {agent_id}: {e}");
            ApiResponse::fail(
    ...
    Ok(ws.on_upgrade(move |socket| client.on_connect(socket).instrument(span)))
}
